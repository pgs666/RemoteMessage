...
- ???????? `REMOTE_MESSAGE_SERVER_BASE_URL`?
- ????????????????? IPv4???????? `169.254.x.x`?
- ?????????`10.x.x.x` / `172.16-31.x.x` / `192.168.x.x`??????? `https://<ip>:<https_port>`?

?????`middle_server/Config/OnboardingQrBootstrap.cs` ? `ResolveServerBaseUrl()`?

### ???????????????

- ???????? `Server Base URL` ? `Client Token`?
- ??? `Device ID` ???????

?????

- `client/flutter_client_source/lib/src/settings_page.dart`
- `_scanOnboardingIntoMainForm()` ??? `_serverCtrl` ? `_passwordCtrl`
- ?????????????? ID ??????

### ??????

1. ?????????? `server.conf` ? `onboarding-qr.txt`?
2. ??????????????????????????
3. ??????? `Device ID`??????????
4. ???????????????/????? `server-cert.cer`?????? CA ?????????
